...
export const buscarAdministradorPorMatricula = async (req: Request, res: Response) => {
    ...
    const httpResponse = await AdministradorService.buscarAdministradorPorMatricula(matricula);
...
export const buscarAdministradorPorId = async (req: Request, res: Response) => {
    const { id } = req.params;
    const httpResponse = await AdministradorService.buscarAdministradorPorIdService(id);
    res.status(httpResponse.statusCode).json(httpResponse.body);
};
