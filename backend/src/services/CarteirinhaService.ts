...
import { v4 as uuidv4 } from 'uuid'; // gera códigos únicos
...
export const emitirCarteirinhaService = async (alunoId: string): Promise<HttpResponseModel> => {
  ...
  try {
    ...
    // Adicionar verificação para garantir que carteirinhasExistentes seja um array
    if (!Array.isArray(carteirinhasExistentes)) {
      console.log('Service: Erro - carteirinhasExistentes não é um array:', carteirinhasExistentes);
      return {
        statusCode: StatusCode.INTERNAL_SERVER_ERROR,
...
export const atualizarStatusCarteirinhaService = async (
  ...
  try {
    ...
    if (status === 'ativa' && carteirinha.status !== 'ativa') {
      ...
      // Adicionar verificação para garantir que carteirinhasDoAluno seja um array
      ...
      // Fix: Usar tipo seguro para acessar _id e converter para string de forma segura
      const temOutraAtiva = carteirinhasDoAluno.some(c => {
        // Garantir que c é um objeto e tem uma propriedade _id
        if (c && typeof c === 'object' && '_id' in c) {
          // Converter o _id para string de forma segura
